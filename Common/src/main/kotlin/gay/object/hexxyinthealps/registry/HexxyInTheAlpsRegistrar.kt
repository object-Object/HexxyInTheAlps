...
typealias RegistrarEntry<T> = HexxyInTheAlpsRegistrar<T>.Entry<out T>

abstract class HexxyInTheAlpsRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HexxyInTheAlps.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexxyInTheAlpsRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
